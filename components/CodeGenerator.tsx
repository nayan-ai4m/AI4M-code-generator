...
export function CodeGenerator({ prompt, onBack }: CodeGeneratorProps) {
  ...
  const generateCode = async () => {
    ...
    try {
      ...
      // Parse the response from OpenAI GPT-4.1
      let generatedFiles;
      try {
        const parsedResponse = JSON.parse(data.processedText);
        generatedFiles = parsedResponse.files || {};
        
        // Store stackblitz config if provided
        const stackblitzConfig = parsedResponse.stackblitzConfig || {
          title: 'Generated Next.js App',
          description: 'Generated by GPT-4.1',
          template: 'nextjs'
        };
        
        console.log('🔗 Stackblitz config:', stackblitzConfig);
      } catch (parseError) {
      ...
      const stackblitzProject = await createStackblitzProject(generatedFiles, parsedResponse?.stackblitzConfig);
...
export default function ${componentName}() {
  ...
  const createStackblitzProject = async (files: GeneratedFiles, config?: any) => {
    ...
    try {
      ...
      const project = {
        files,
        title: config?.title || 'Generated Next.js App',
        description: config?.description || 'Generated by GPT-4.1 AI Code Generator',
      ...
      const projectId = `gpt4-generated-${Date.now()}`;
      
      // Open project in new tab for live preview
      const vm = await StackBlitz.openProject(project, {
      ...
      const stackblitzUrl = `https://stackblitz.com/~/github.com/stackblitz/bolt.new`;
          ...
          <CardContent className="p-8">
            <div className="flex items-center space-x-4 mb-6">
              ...
              <div>
                ...
                <p className="text-blue-700 dark:text-blue-300">
                  GPT-4.1 is creating a production-ready Next.js application with Tailwind CSS
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 text-center text-sm">
              ...
              <div className={`p-3 rounded-lg ${generationProgress > 60 ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400'}`}>
                <div className="font-semibold">GPT-4.1 Generating Code</div>
