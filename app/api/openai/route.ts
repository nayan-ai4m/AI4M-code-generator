...
- Comments highlighting key changes`;
    ...
    const processedText = completion.choices[0]?.message?.content || 'No response generated';
    ...
    console.error('OpenAI API error stack:', error instanceof Error ? error.stack : 'No stack trace');
    ...
    if (error instanceof Error) {
      if (error.message.includes('API key')) {
        ...
        errorMessage = 'The requested model is not available. Please contact support.';
      } else if (error.message.includes('network') || error.message.includes('fetch')) {
        errorMessage = 'Network connection to OpenAI API failed. Please check your internet connection and try again.';
      }
    }
